...
public final class PlayerESP extends Module {
    ...
    public void onRender3D(final Render3DEvent render3DEvent) {
        for (final Object next : this.mc.world.getPlayers()) {
            if (next != this.mc.player) {
                ...
                // Use Uranium Green color
                Color espColor = new Color(URANIUM_GREEN.getRed(), URANIUM_GREEN.getGreen(), URANIUM_GREEN.getBlue(), alpha.getIntValue());
                RenderUtils.renderFilledBox(render3DEvent.matrixStack, (float) lerp - ((PlayerEntity) next).getWidth() / 2.0f, (float) lerp2, (float) lerp3 - ((PlayerEntity) next).getWidth() / 2.0f, (float) lerp + ((PlayerEntity) next).getWidth() / 2.0f, (float) lerp2 + ((PlayerEntity) next).getHeight(), (float) lerp3 + ((PlayerEntity) next).getWidth() / 2.0f, espColor);
                
                if (this.tracers.getValue()) {
                    // Use Uranium Green for tracers
                    Color tracerColor = new Color(URANIUM_GREEN.getRed(), URANIUM_GREEN.getGreen(), URANIUM_GREEN.getBlue(), 255);
                    RenderUtils.renderLine(render3DEvent.matrixStack, tracerColor, this.mc.crosshairTarget.getPos(), ((PlayerEntity) next).getLerpedPos(RenderTickCounter.ONE.getTickDelta(true)));
                }
    ...
    private Color getColorWithAlpha(final int a) {
        // Override to always use Uranium Green
        return new Color(URANIUM_GREEN.getRed(), URANIUM_GREEN.getGreen(), URANIUM_GREEN.getBlue(), a);
    }
}
